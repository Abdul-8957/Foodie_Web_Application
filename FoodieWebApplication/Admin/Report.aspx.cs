...
namespace FoodieWebApplication.Admin
{
    public partial class Report : System.Web.UI.Page
    {
        ...
        private void GetReports(DateTime fromDate, DateTime toDate)
        {
            ...
            cmd = new SqlCommand("SellingReportSp", con);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblTotal.Text = "Sold Cost: " + grandTotal + " Rupees";
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Write("<script>alert('ToDate cannot be greater then current date!');</script>");
            }
            else if (fromDate>toDate)
            {
                Response.Write("<script>alert('FromDate cannot be greater then ToDate!');</script>");
            }
            else
            {
